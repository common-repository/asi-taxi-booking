<?php
      ...
      if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['addrate']) ) 
        {  
             ...
             $diskmmile=sanitize_text_field($_POST['diskmmile']);
             $curr=sanitize_text_field($_POST['curr']);
             $bcolor=sanitize_text_field($_POST['bcolor']);
             $table_name = $wpdb->prefix."fare";
             $wpdb->query($wpdb->prepare("UPDATE $table_name SET mile=%s,stop=%s,seat=%s,adul=%s,inf=%s,lugg=%s,minute=%s,diskmmile=%d,curr=%s,color=%s WHERE fare_id=%d",$mile,$stop,$seat,$adul,$inf,$lugg,$minute,$diskmmile,$curr,$bcolor,1));
        }
    ...
    class asi_taxibook_plugin_admin
    {
         /** verbingo_plugin father class */
            ...
            function asi_all_orders()
            {
                global $wpdb;
                $bookingt = $wpdb->prefix."Booking";
                $bookings = $wpdb->get_results("SELECT * FROM $bookingt",ARRAY_A);
                return $bookings;
            }
            /** UTILITY FUNCTIONS * */
        ...
        function register_asi_taxi_settings() 
            ...
            { 
                ...
                if($fares[0]['diskmmile']==0){ $kmselected="";$milesselected="selected";}else{$milesselected="";$kmselected="selected";}
                $selectdistance='<select name="diskmmile" id="diskmmile" style="width:105px;">';
                $selectdistance.='<option value="0" '.$milesselected.'>Miles</option><option value="1" '.$kmselected.'>KM</option></select>';
                ...
                echo '<tr><td>Type of distance</td><td>'.$selectdistance.'</td></tr>';                
        ...
        echo '<tr><td colspan="4" style="border:none !important;"><input type="submit" id="carsubmit" value="Save Changes" class="button-primary" name="addtaxi" style="margin-top:40px;width:105px;z-index:2147483647; padding: 0px;"/></td></tr>';
    ...
    { 
        $booking=$this->asi_all_orders();
        echo '<br><table>';
        $i=1;
        echo '<thead><tr class="home"><th>S.No</th><th>Name</th><th>Email</th><th>Cell</th><th>PickUp</th><th>Drop Off</th><th>Time</th><th>Car Type</th><th>Adults</th><th>Baby</th><th>Infants</th><th>Luggage</th><th>Delete</th></tr></thead><tbody>';
        if(empty($booking))
        {
        ...
        else
        {
            ...
            {    
               echo '<tr><td>'.$i.'</td><td>'.$order['name'].'</td><td>'.$order['email'].'</td><td>'.$order['cell'].'</td><td>'.$order['pickup'].'</td>
               <td>'.$order['dropoff'].'</td><td>'.$order['date'].'</td><td>'.$order['cartype'].'</td><td>'.$order['adults'].'</td><td>'.$order['baby'].'</td><td>'.$order['infan'].'</td><td>'.$order['lugg'].'</td><td><div class="actions"><img  class="rembook" title="Remove" alt="Delete" src="'.plugins_url("img/", __FILE__).'/delete.png" content="'.$order['bk_id'].'">
               </a></div></td></tr>';
               $i++;
            }
        }       
